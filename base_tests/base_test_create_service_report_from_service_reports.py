...
FIELD_ORDER = ["report_type", "internal_only", "name", "reportDate", "ticket_id", "assignees"]
FIELD_TYPES = {
...
def open_form(page: Page):
    ...
    base_test_create_service_report_from_service_reports_recorded.py — edit selectors here if the
...
def set_field(page: Page, field: str, value):
    ...
    if field == "report_type":
        ...
        _select_option(page, page.get_by_role("combobox", name="Select Assignee(s)"), value)
    else:
...
def read_field(page: Page, field: str):
    ...
    if field == "report_type":
        ...
        return (page.get_by_role("combobox", name="Select Assignee(s)")).text_content() or ""
    else:
        raise ValueError(f"Unknown field: {field}")
...
def open_field(page: Page, field: str):
    ...
    if field == "report_type":
        ...
        _open_dropdown(page, page.get_by_role("combobox", name="Select Assignee(s)"))
    else:
        raise ValueError(f"Unknown field: {field}")
...
def is_field_present(page: Page, field: str) -> bool:
    ...
    if field == "report_type":
        ...
        return (page.get_by_role("combobox", name="Select Assignee(s)")).count() > 0
    else:
        raise ValueError(f"Unknown field: {field}")
...
def run_test(page: Page, inputs: dict, expected_outcome: str, expected_message: str):
    """
    Execute one create_service_report/create_service_report_from_service_reports scenario.

    inputs keys : report_type, internal_only, name, reportDate, ticket_id, assignees
