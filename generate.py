...
MANDATORY DISABLE/LOCK CHECK — perform this for every B-item you write, without exception:

  Step 1 — Is the target field disabled or non-interactive when the trigger HAS a value?
           Look for: disabled prop, aria-disabled, readOnly, MUI InputProps={{readOnly}},
           or pointer-events:none on the field or its wrapper — conditional on the trigger
           being set. Also covers disabled={someArray.length <= 0} where someArray is
           populated by the trigger's cascade (array is non-empty when trigger is set →
           field is enabled, empty when trigger is absent → field is disabled).
           If YES → change this B-item's EFFECT to "lock".

  Step 2 — Is the target field disabled or non-interactive when the trigger has NO value?
           Same patterns as above, but the condition fires when trigger is absent/empty.
           If YES → write a SECOND B-item: same trigger and target, EFFECT: "disable".

These two checks are required. Do not proceed to the next B-item until both are done.
