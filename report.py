...
Reads results/raw_results_{flow}.json and matrices/scenario_matrix_{flow}.json
(the exact paths execute.py writes to / reads from) and writes a structured
...
Usage:
    python testGen/report.py --flow create_service_report
    python testGen/report.py --flow create_service_report --out path/to/report.xlsx
"""
...
# actual_outcome values that mean "the app did the positive/permissive thing"
# vs the negative/blocked thing vs a run that never produced a real answer.
# See execute.py's _detect_outcome() / _compute_passed() for the full vocabulary.
_POSITIVE_OUTCOMES = {
    "submitted", "incorrectly accepted", "control accessible",
    "redirected to login", "not authorized shown", "access granted",
}
_NEGATIVE_OUTCOMES = {
    "correctly rejected", "incorrectly rejected", "rejected",
    "control not accessible", "ambiguous", "error", "timeout",
}


def _app_behavior_label(actual_outcome: str | None) -> str:
    """Map execute.py's actual_outcome to a simple PASS / FAIL / SKIP label
    for the App Behaviour column — this reflects what the app *did*, not
    whether that matched the expected scenario (that's Scenario Result)."""
    v = str(actual_outcome or "").strip().lower()
    if not v or v == "skip":
        return "SKIP"
    if v in _POSITIVE_OUTCOMES:
        return "PASS"
    if v in _NEGATIVE_OUTCOMES:
        return "FAIL"
    return "FAIL"
...
#
# scenario dict keys, per execute.py's run_scenario()/load_matrix():
#   case, category, flow_type, persona, title, description, inputs,
#   expected_outcome, expected_result, preconditions, skip, skip_reason,
#   session_count
# There is no "flow" (it's implicit — one matrix file per flow), no "role"
# (it's "persona"), and no "id"/"field"/"business_rule"/"test_mode" — those
# were columns from a different pipeline's matrix format. We fall back
# gracefully in case a future matrix version adds any of them.

def write_scenario_matrix(wb: Workbook, scenarios: list[dict], flow: str):
    ...
    for r_idx, s in enumerate(scenarios, 2):
        alt = r_idx % 2 == 0
        row = [
            s.get("case") or s.get("id", ""),
            s.get("category", ""),
            s.get("flow") or flow,
            s.get("persona") or s.get("role", ""),
            s.get("test_mode") or s.get("flow_type", ""),
            s.get("description", ""),
            _inputs_str(s.get("inputs")),
            s.get("field") or s.get("field_under_test") or "",
            s.get("expected_outcome", ""),
            s.get("expected_result") or s.get("expected_message") or "",
...
#
# result dict keys, per execute.py's run_scenario()/_skip():
#   case, category, flow, flow_type, persona, title, description, inputs,
#   expected_outcome, expected_message, actual_outcome, actual_message,
#   passed, raw_output, artifacts, duration_ms
...
def write_test_results(wb: Workbook, results: list[dict], run_ts: str) -> int:
    ...
    for r_idx, r in enumerate(results, 2):
        ...
        # Extract the most useful snippet from raw pytest/playwright output
...
def _load_matrix(flow: str) -> list[dict]:
    """Same lookup logic as execute.py's load_matrix() — exact filename
    first, falls back to a glob match if versioned/suffixed filenames exist."""
    ...
    return raw.get("scenarios", raw) if isinstance(raw, dict) else raw
...
def main():
    ...
    results_path = RESULTS_DIR / f"raw_results_{args.flow}.json"
    ...
    scenarios = _load_matrix(args.flow)
    if not scenarios:
        print(f"  WARN: no scenario matrix found for '{args.flow}' in {MATRICES_DIR} "
              f"— Scenario Matrix sheet will be empty.")

    wb = Workbook()
    wb.remove(wb.active)

    write_summary(wb, results, args.flow, run_ts)
    write_scenario_matrix(wb, scenarios, args.flow)
